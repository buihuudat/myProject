...
$(document).ready(function () {
  ...
  if (myUser != null || myUser != undefined) {
    ...
    if (r == "true") {
      $(".username").val(localStorage.getItem("usernamebookstore"));
      $(".password").val(localStorage.getItem("passwordbookstore"));
    }
  }
...
var temp=cvt_hex(H0)+cvt_hex(H1)+cvt_hex(H2)+cvt_hex(H3)+cvt_hex(H4);return temp.toLowerCase();}
  ...
  $(".btnsignup").click(function () {
    ...
    if (fullname == "") {
      ...
      alert("Vui lòng nhập mật khẩu");
    } else if (passwordsu.length <= 7) {
      alert("Yêu cầu nhập mật khẩu ít nhất 8 kí tự");
    } else {
      var dataSend = {
        event: "signup",
        fullname: fullname,
        ...
        passwordsu: SHA1(passwordsu),
      };
      queryData("asset/php/api.php", dataSend, function (data) {
        if (data.success == 1) {
          var alert = window.confirm('Đăng ký thành công. Bạn có muốn đăng nhập không?');
          alert? container.classList.remove("sign-up-mode"):container.classList.add("sign-up-mode");
        } else {
